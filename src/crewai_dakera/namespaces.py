...
class DakeraNamespaceManager:
    ...
    def create(self, name: str, *, dimension: int | None = None, metric: str | None = None, **kwargs: Any) -> dict[str, Any]:
        """Create a namespace."""
    ...
    def get(self, name: str) -> dict[str, Any]:
        ...
        return {"name": result.name, "dimension": result.dimension, "metric": result.metric, "vector_count": result.vector_count}

    def list(self) -> list[dict[str, Any]]:
        """List all namespaces."""
        result = self._client.list_namespaces()
        return [{"name": ns.name, "dimension": ns.dimension, "vector_count": ns.vector_count} for ns in result.namespaces]
